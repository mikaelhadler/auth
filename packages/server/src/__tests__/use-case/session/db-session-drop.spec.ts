...
import {
  ...
  makeAuthenticationUpdateStatusRepositoryStub,
...
describe("DbSessionDrop", () => {
  ...
  it("should throw if session drop throws", async () => {
    ...
    const { sut, sessionDropStub } = makeSut();
    jest
      .spyOn(sessionDropStub, functionName)
      .mockReturnValueOnce(Promise.reject(expectedThrow));
    const promise = sut.drop(sessionId, accountId);
    await expect(promise).rejects.toThrowError(expectedThrow);
  });
  ...
  it("should update authentication status to offline if not has other session opened", async () => {
    ...
    jest
      ...
      .mockReturnValueOnce(Promise.resolve(returnCountSession));
    const spy = jest.spyOn(authenticationUpdateStatus, functionSpyName);
    await sut.drop(sessionId, accountId);
    expect(spy).toHaveBeenCalledWith(
      mockedAuthentication.id,
...
function makeSut(): SutTypes {
  ...
  const sessionCountByAuthenticationStub =
    makeSessionCountByAuthenticationRepository();
  const authenticationUpdateStatus =
    makeAuthenticationUpdateStatusRepositoryStub();
  const sut = new DbSessionDrop(
